...
# Use webroot so wir behalten die Kontrolle über die vHost-Datei
run_certbot_webroot(){
...
if [[ "$WEB" == "nginx" ]]; then
  # 1) HTTP-only vHost (funktioniert sofort; ACME webroot erreichbar)
  configure_nginx_http_only

  # 2) SSL optional: Zert holen und dann SSL-vHost schreiben (dein Template)
  if ask_certbot; then
    install_certbot_pkgs
    run_certbot_webroot
    configure_nginx_ssl
  ...
  # Apache-Flow bleibt wie bisher (keine Änderungen)
  # vHost & FPM werden in app_install_steps indirekt genutzt; falls gewünscht, kann SSL via ask_certbot() laufen
  if ask_certbot; then
    install_certbot_pkgs
    # Für Apache nehmen wir das Plugin direkt:
